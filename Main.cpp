#include "Infer.h"
#include <cstdlib>
//#include <cmath>
#include <string> 

int main(int argc, char const *argv[])
{
    Detector* detector = new Detector;
    string xml_path = "/home/slava/YoloV5CppInfer/FCB_IR_Models/S_Model/best_FP16.xml";
    detector->init(xml_path, 0.1, 0.5, "CPU");
    string input_video = "/home/slava/YoloV5CppInfer/outpy.avi";

    // cv::VideoCapture capture;
    // capture.open(input_video);

    // Mat src;

    // while(1){
    //     capture >> src;

    //     vector<Detector::Object> detected_objects;

    //     auto start = chrono::high_resolution_clock::now();
    //     detector->process_frame(src, detected_objects, 320);
    //     auto end = chrono::high_resolution_clock::now();
    //     std::chrono::duration<double> diff = end - start;
    //     double fps = 1/diff.count();

    //     std::cout << "---new frame---" << std::endl;

    //     for(int i=0;i<detected_objects.size();++i){
    //         int xmin = detected_objects[i].rect.x;
    //         int ymin = detected_objects[i].rect.y;
    //         int width = detected_objects[i].rect.width;
    //         int height = detected_objects[i].rect.height;
    //         float prob = detected_objects[i].prob;

    //         Rect rect(xmin, ymin, width, height);
    //         std::cout << "xmin: " << xmin << " ymin: " << ymin << " width: " << width <<  " height: " << height << " prob: " << prob << std::endl;
    //         cv::rectangle(src, rect, Scalar(255, 0, 0), 1, LINE_8, 0);
    //         cv::putText(src, to_string(round(prob*100)/100) + " %", cv::Point(xmin, ymin), cv::FONT_HERSHEY_DUPLEX, 1.0, (0, 255, 0), 2);
    //     }

    //     imshow("output",src);
    //     waitKey(1);
    // }


    Mat src = imread("face.png");
    Mat osrc = src.clone();
    resize(osrc,osrc,Size(320,320));
    vector<Detector::Object> detected_objects;
    auto start = chrono::high_resolution_clock::now();
    detector->process_frame(src, detected_objects, 320);
    auto end = chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
    cout<<"use "<<diff.count()<<" s" << endl;
    for(int i=0;i<detected_objects.size();++i){
        int xmin = detected_objects[i].rect.x;
        int ymin = detected_objects[i].rect.y;
        int width = detected_objects[i].rect.width;
        int height = detected_objects[i].rect.height;
        Rect rect(xmin, ymin, width, height);
        cv::rectangle(osrc, rect, Scalar(0, 0, 255),1, LINE_8,0);
    }
    imshow("result",osrc);
    waitKey(0);
}

    /*
    VideoCapture capture;
    capture.open(0);
    Mat src;
    while(1){
        capture >> src;
        vector<Detector::Object> detected_objects;
    detector->process_frame(src,detected_objects);
    for(int i=0;i<detected_objects.size();++i){
         int xmin = detected_objects[i].rect.x;
        int ymin = detected_objects[i].rect.y;
        int width = detected_objects[i].rect.width;
        int height = detected_objects[i].rect.height;
        Rect rect(xmin, ymin, width, height);//左上坐标（x,y）和矩形的长(x)宽(y)
        cv::rectangle(src, rect, Scalar(255, 0, 0),1, LINE_8,0);
    }
        imshow("cap",src);
        waitKey(1);
    }
    */
    // Mat src = imread("../res/bus.jpg");
    // Mat osrc = src.clone();
    // resize(osrc,osrc,Size(640,640));
    // vector<Detector::Object> detected_objects;
    // auto start = chrono::high_resolution_clock::now();
    // detector->process_frame(src,detected_objects);
    // auto end = chrono::high_resolution_clock::now();
    // std::chrono::duration<double> diff = end - start;
    // cout<<"use "<<diff.count()<<" s" << endl;
    // for(int i=0;i<detected_objects.size();++i){
    //      int xmin = detected_objects[i].rect.x;
    //     int ymin = detected_objects[i].rect.y;
    //     int width = detected_objects[i].rect.width;
    //     int height = detected_objects[i].rect.height;
    //     Rect rect(xmin, ymin, width, height);//左上坐标（x,y）和矩形的长(x)宽(y)
    //     cv::rectangle(osrc, rect, Scalar(0, 0, 255),1, LINE_8,0);
    // }
    // imshow("result",osrc);
    // waitKey(0);
