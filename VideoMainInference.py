...
class YoloParams:
    ...
    def __init__(self,  side):
        ...
        self.classes = 3 #if 'classes' not in param else int(param['classes'])80
        self.side = side
...
def letterbox(img, size=(imsize, imsize), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
...
def scale_bbox(x, y, height, width, class_id, confidence, im_h, im_w, resized_im_h=imsize, resized_im_w=imsize):
...
def main():
    ...
    device = "GPU"
    model = "/home/slava/Source/YoloV5_OVN_Inference/NewUbuntuConv/ColabSModel/best.xml"
    input_video = "outpy.avi"
    prob_threshold = 0.5
    iou_threshold = 0.4
...
#            sys.exit(1)
    ...
    out = cv2.VideoWriter('V5outpy.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (imsize,imsize))
    ...
    while cap.isOpened():
        ...
        if exec_net.requests[cur_request_id].wait(-1) == 0:
            ...
            for layer_name, out_blob in output.items():
                ...
                objects += parse_yolo_region(out_blob.buffer, in_frame.shape[2:], frame.shape[:-1], layer_params, prob_threshold)
        ...
        for obj in objects:
            ...
            cv2.putText(frame, "fps: " + str(fc/(time() - start_time)), (20, 20), cv2.FONT_HERSHEY_COMPLEX, 0.6, (0 ,0, 0), 1)

        cv2.imshow("DetectionResults", frame)

        rframe = cv2.resize(frame, (imsize, imsize), interpolation=cv2.INTER_LINEAR)
        out.write(rframe)
